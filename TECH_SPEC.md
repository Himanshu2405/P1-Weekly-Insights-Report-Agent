...
| 2 | Weekly Gross Revenue (excl. cancelled) | Sum of `order_items.sale_price` for items created in the week with item status != Cancelled. Returns NOT subtracted (tracked by KPI 5) | `order_items` | Up | Definition fidelity: LLM must not call it "net" or imply returns are removed |
...
The data brief is the ONLY data the LLM sees. It is generated by code every run, saved as `briefs/brief_<week_start>.json`, and frozen copies become golden-set fixtures. Full illustrative example: [briefs/example_brief.json](briefs/example_brief.json) (mockup numbers, not real data).
...
| `so_what_facts` | Pre-computed implications | `next_week_targets`, `full_quarter_plan`, `q3_remaining_to_plan`, `qtd_attainment_excl_flagged_pct`, `revenue_per_pp_cancellation`, `avg_8wk`, `top_contributor`, `yoy_vs_plan_growth`, `signups_vs_orders_growth`, `segments_all_growing` |
